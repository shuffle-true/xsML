...
def compute_biase_variance(regressor, X: np.array, y, num_runs=1000):
    """
    ...
    for i in range(num_runs):
        ...
        X_test = X[oob_]
        prediction = np.append(prediction, regressor.predict(X_test))
        test = np.append(test, mse(y[oob_], regressor.predict(X_test)))
    ...
    # усредненное смещение и разброс по всем объектам - искомая величина
